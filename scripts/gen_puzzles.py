...
def accepted_anagrams(word, groups):
    """All dictionary words that are a full anagram of ``word`` (same letter
    multiset), uppercased and sorted, INCLUDING ``word`` itself.

    Used to enforce solution UNIQUENESS at play time: the pooled letters of the
    two source words may spell more than one valid final word. When they do, the
    puzzle's answer is not unique, so every such anagram is an accepted answer
    (the player is credited for any of them). Length always matches len(word),
    so alternatives drop into the exact same answer boxes.
    """
    words = groups.get(signature(word), [word])
    return sorted({w.upper() for w in words} | {word.upper()})
...
def build_puzzles(defs, selected, groups):
    puzzles = []
    for offset, t in enumerate(selected):
        day = FIRST_DAY + offset            # true index; may be negative
        d = ANCHOR_DATE + timedelta(days=day)
        # Every dictionary word the pooled letters can spell (>= 1: the answer
        # itself). len(accept) > 1 => the solution is NOT unique; the game
        # credits any of these as a correct final answer.
        accept = accepted_anagrams(t["c"], groups)
        puzzles.append({
            "day": day,
            "date": d.isoformat(),
...
def _write_days_js(puzzles):
    ...
    out = (
        ...
        "b:{word,clue}, c:{word,clue}, accept:[...] }.\n"
...
def main():
    ...
    puzzles = build_puzzles(defs, selected, groups)

    # Uniqueness report: days whose pooled letters spell more than one valid
    # word. Those days accept any of the anagrams as a correct final answer.
    non_unique = [p for p in puzzles if len(p["accept"]) > 1]
    print(f"[gen] non-unique solutions (accept > 1): {len(non_unique)}/"
          f"{len(puzzles)}")
    for p in non_unique[:15]:
        alts = [w for w in p["accept"] if w != p["c"]["word"]]
        print(f"[gen]   day {p['day']:>4} {p['c']['word']:<12} "
              f"also accepts: {', '.join(alts)}")
    if len(non_unique) > 15:
        print(f"[gen]   ... and {len(non_unique) - 15} more")
